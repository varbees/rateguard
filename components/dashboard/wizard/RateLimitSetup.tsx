...
// Backend validation constraints from models/api_config.go
export const BACKEND_LIMITS = {
  ratePerSecond: { min: 1, max: 10000 },
  burstSize: { min: 1, max: 20000 },
  ratePerHour: { min: 0, max: 1000000 },
  ratePerDay: { min: 0, max: 10000000 },
  ratePerMonth: { min: 0, max: 100000000 },
  timeoutSeconds: { min: 1, max: 300 },
  retryAttempts: { min: 0, max: 5 },
};

export function RateLimitSetup({
  state,
  updateState,
  onNext,
  onBack,
  ...
  // Plan-based limits (Free: 10 req/s, Pro/Enterprise: 100 req/s)
    ...
    <div className="space-y-6 max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
      ...
      <div className="space-y-4">
        <div className="flex justify-between">
          ...
          <span className="text-sm text-muted-foreground">
            {state.rate_limit_per_second} req/s
          </span>
        </div>
        <div className="flex items-center gap-4">
          <Slider
            ...
            onValueChange={([value]) =>
              updateState({ rate_limit_per_second: value })
            }
            max={maxPerSecond}
            min={1}
            step={1}
          ...
          <Input
            ...
            value={state.rate_limit_per_second}
            onChange={(e) =>
              updateState({ rate_limit_per_second: Number(e.target.value) })
            }
            className="w-20"
            min={1}
            max={maxPerSecond}
      ...
      <div className="space-y-4">
        ...
        <div className="flex items-center gap-4">
          ...
          <Input
            ...
            value={state.burst_size}
            onChange={(e) =>
              updateState({ burst_size: Number(e.target.value) })
            }
            className="w-20"
          />
        </div>
        <p className="text-xs text-muted-foreground">
          Allows brief spikes above the rate limit. Recommended:{" "}
          {state.rate_limit_per_second * 2}.
        </p>
      </div>
      ...
      >
        <CollapsibleTrigger asChild>
          <Button
            variant="ghost"
            className="flex w-full justify-between p-0 h-auto font-normal hover:bg-transparent"
          >
        ...
        <CollapsibleContent>
          {isPro ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateState({
                        rate_limit_per_hour: e.target.value
                ...
                <div className="space-y-2">
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      updateState({
                        rate_limit_per_month: e.target.value
                          ? Number(e.target.value)
                          : undefined,
                      })
                    }
            ...
            <Alert variant="default" className="mt-4 bg-muted/50">
              ...
              <AlertDescription>
                <strong>Pro Feature:</strong> Set hourly, daily, and monthly
                limits to control costs and usage.
                <Link
      ...
      <div className="p-4 bg-muted/50 rounded-lg border">
        ...
        <ul className="text-sm space-y-1 text-muted-foreground">
          ...
          {state.rate_limit_per_hour && (
            <li>✓ Max {state.rate_limit_per_hour} requests per hour</li>
          )}
          {state.rate_limit_per_day && (
            <li>✓ Max {state.rate_limit_per_day} requests per day</li>
          )}
        </ul>
      </div>
