...
export default function WebhooksPage() {
  return (
    <div className="space-y-8">
      ...
      <Callout title="Reliability Without the Theater" type="default">
        Our webhook system uses PostgreSQL for persistence and exponential
        backoff for retries. It is built to be available wherever webhooks are enabled.
      </Callout>
              ...
              examples={[
                {
                  label: "cURL",
                  language: "bash",
                  code: `curl -X POST ${API_BASE_URL}/webhook/inbox \\
  ...
  -d '{
    ...
    "target_url": "https://your-api.com/webhooks"
                ...
                },
                {
                  label: "JavaScript",
                  language: "javascript",
                  code: `const response = await fetch('${API_BASE_URL}/webhook/inbox', {
  method: 'POST',
  headers: {
...
curl ${API_BASE_URL}/webhook/status?status=failed \\
  -H "Authorization: Bearer YOUR_TOKEN"

# Get specific webhook
curl ${API_BASE_URL}/webhook/events/:id \\
  -H "Authorization: Bearer YOUR_TOKEN"`}
            />
          </div>
