...
export default function QueueAnalytics({
  ...
  const queuedByApi: APIQueue[] = stats.queued_by_api ?? [];
  ...
  const hasQueuedRequests = queuedByApi.some((api) => api.queued_requests > 0);
        ...
        >
          ...
          <TabsContent value="requests" className="py-4">
            <div className="h-[300px] w-full">
              {queuedByApi.length > 0 ? (
                <ResponsiveContainer width="100%" height="100%">
                  <BarChart
                    data={prepareBarData(queuedByApi)}
            ...
            <div className="h-[300px] w-full">
              {hasQueuedRequests ? (
                <ResponsiveContainer width="100%" height="100%">
                  <PieChart>
                    <Pie
                      data={preparePieData(queuedByApi)}
                      cx="50%"
                      cy="50%"
                      labelLine={true}
                      label={({ name, percent }) =>
                      ...
                      dataKey="value"
                    >
                      {preparePieData(queuedByApi).map(
                        (entry, index) => (
                          <Cell
                            key={`cell-${index}`}
