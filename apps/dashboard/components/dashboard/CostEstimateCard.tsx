...
import { dashboardAPI, type CostEstimate } from "@/lib/api";
import { useWebSocket } from "@/lib/websocket/context";
import { DollarSign, TrendingUp, Calendar, Zap } from "lucide-react";

export function CostEstimateCard() {
  const { data, isLoading, refetch } = useQuery<CostEstimate>({
      ...
      <CardContent>
        <div className="space-y-4">
          ...
          {data.api_costs && data.api_costs.length > 0 && (
            <div className="pt-3 border-t border-gray-200 dark:border-gray-700">
              ...
              <div className="space-y-1">
                {data.api_costs.slice(0, 3).map((apiCost: { api_id: string; api_name: string; total_cost: number }) => (
                  <div
                    key={apiCost.api_id}
          ...
          <div className="pt-2 text-[10px] text-gray-500 dark:text-gray-500">
            {hasTokenData
              ? "Usage model: request-based + token-based accounting"
              : "Request-based usage estimate"}
          </div>
        </div>
      </CardContent>
    </Card>
