...
export function StreamingCostCalculator({
  ...
  const [preset, setPreset] = useState<"starter" | "standard" | "strict">("standard");
  const calculations = useMemo(() => {
    ...
    const baseCostPerGB = {
      starter: 0.15, // Higher cost
      standard: 0.10, // Standard
      strict: 0.08, // Volume discount
    };
    ...
    // Calculate bandwidth cost based on the selected preset
    const bandwidthCost =
      totalDataGB * baseCostPerGB[preset] + totalDataGB * streamingCostPerGB[preset];

    // RateGuard preset costs
    const presetCosts = {
      starter: 0,
      standard: 19,
      strict: 99,
    };
    const rateGuardCost = presetCosts[preset];
  ...
  }, [monthlyStreams, avgResponseKB, preset]);
    ...
    >
      ...
      <div className="space-y-4 mb-6">
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {(["starter", "standard", "strict"] as const).map((presetType) => (
              <button
                key={presetType}
                onClick={() => setPreset(presetType)}
      ...
      <div className="border-t border-border pt-6 space-y-4">
        ...
        <div className="space-y-2">
          ...
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">RateGuard ({preset}):</span>
