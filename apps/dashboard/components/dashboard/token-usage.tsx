...
export function TokenUsageWidget() {
  ...
  const { data: tokenData, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      const presetTier = dashboardData?.preset?.tier ?? "dev";
      const preset =
        presetTier === "standard"
          ? "standard"
          : presetTier === "high-throughput" ||
              presetTier === "llm-heavy" ||
              presetTier === "strict-upstream-protection"
            ? "strict"
            : "starter";
      ...
      return {
        used: Math.floor(Math.random() * limits[preset] * 0.7), // Mock 70% usage
        limit: limits[preset],
        preset,
      };
    },
            ...
            <div className="flex items-start gap-2">
              ...
              <div className="flex-1 space-y-2">
                ...
                <p className={`text-xs ${isOverLimit ? "text-red-800 dark:text-red-200" : "text-orange-800 dark:text-orange-200"}`}>
                  {isOverLimit 
                    ? "You've exceeded your monthly token quota. Review your preset or request budget." 
                    : "You're approaching your token limit. Review your preset or request budget."}
                </p>
